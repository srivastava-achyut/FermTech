...
export default function Register() {
  ...
  return (
    <section className="grid two">
      <div>
        ...
        <p className="section-lead">Create a farmer or buyer account to access fermtech services admin account is restricted to use</p>
      </div>
  ...
  <option value="buyer">Buyer</option>
        ...
        <select value={form.language} onChange={(e) => setForm({ ...form, language: e.target.value })}>{languages.map((language) => <option key={language.code} value={language.code}>{language.label}</option>)}</select></div>
